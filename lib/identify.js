...
function _is_email_busy(client) {
    let {req} = client;
    return {
...
class Ident {
    ...
    async get_antibot_task(client) {
        ...
        return JSON.stringify({ip, task})
    }

    async is_user(client) {
        let {cookie} = client;
        if (cookie && cookie[token_name]) return await check_user_token(cookie[token_name]); // {user_id, user_level} | false
        return false;
    }

    async register_user(client) {
        let {body, send} = client;
        let {email, password, id_task_antibot, answer_antibot} = body;

        if (!_check_answer_antibot(id_task_antibot, answer_antibot)) {
            send(401, 'check_bot_error', 'check_bot_error');
            return;
        }
        let message = error_valid_register_data(email, password);
        if (message) {
            send(401, message, message)
            return;
        }
        if (_is_email_busy(client)) {
            send(401, 'email_taken', 'email_taken')
            return;
        }
        password = await _resolve_password(password);
        let new_user = {
        ...
        send(401, 'error_db', 'error_db');
    }

    async auth_user(client) {
        let {body, send, res} = client;
        let {email, password, id_task_antibot, answer_antibot} = body;
        if (!_check_answer_antibot(id_task_antibot, answer_antibot)) {
            send(401, 'error_db', 'error_db');
            return;
        ...
        send(401, 'auth_error', 'auth_error');
    }
}
