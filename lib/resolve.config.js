...
function resolve_urls(urls) {
    urls.forEach(url => {
        ...
        if (typeof url.access_level_only !== "number" || isNaN(url.access_level_only)) url.access_level_only = null;
        if (typeof url.access_level !== "number" || isNaN(url.access_level)) url.access_level = url.access_level_only;
    })
}
